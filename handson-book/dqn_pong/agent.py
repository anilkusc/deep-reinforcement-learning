...
from torch.nn import Sequential,Linear,LeakyReLU,ReLU,MSELoss,Conv2d,Flatten
...
class Agent(nn.Module):
    ...
    def model_constructor(self):
        ...
        self.conv = Sequential(
            ...
            ReLU()
            #Conv2d(self.conv_hidden * 2, self.conv_hidden * 2, kernel_size=4, stride=1),
            #ReLU()
            ).to(self.device)
    ...
    def compute_loss(self):
        ...
        current_q_value = self.forward(state_batch).gather(1, action_batch.unsqueeze(1)).squeeze(1)
        with torch.no_grad():
